...
let DInvariants = new Map();
class CurvePool extends types_1.Pool {
    constructor(poolId, dexId, tokens, reserves, fee, A) {
        super(poolId, dexId, tokens);
        this.reserves = reserves;
        this.startingReserves = [...this.reserves];
        this.fee = new bignumber_js_1.default(fee);
        this.amplificationCoeff = new bignumber_js_1.default(A);
    }
    reset() {
...
*/
function calculateOutputAmount(pool, tokenA, tokenB, dx) {
    ...
    const precisions = pool.tokens.map((token) => new bignumber_js_1.default(10 ** (maxDecimals - token.decimals)));
...
*/
function getYAfterSwap(pool, i, j, x, amp) {
    ...
    if (DInvariants.has(hash))
        return DInvariants.get(hash);
    // init params
    const N_COINS = pool.tokens.length;
    const Ann = amp.times(N_COINS);
    const D = calculateDInvariant(pool, amp);
    let coeff = D;
    let currentX = new bignumber_js_1.default(0); //token amount associated with the current iteration in the loop
    ...
    let y = new bignumber_js_1.default(D);
...
*/
function calculateDInvariant(pool, amp) {
    // check hash if it has already been calculated
    ...
    let D = sum;
    let prevD = new bignumber_js_1.default(0);
    const nCoins = pool.tokens.length;
    let Ann = amp.times(nCoins);
    if (sum.eq((0, bignumber_js_1.default)(0)))
...
function calculateDHash(pool, amp) {
    const values = sortLargeNumbers(pool.reserves);
    values.push(amp.toFixed().toString());
    const valuesString = values.join('');
    const hash = (0, js_sha256_1.sha256)(valuesString);
    return hash;
}
function calculateYHash(pool, i, j, x, amp) {
    const values = sortLargeNumbers(pool.reserves);
